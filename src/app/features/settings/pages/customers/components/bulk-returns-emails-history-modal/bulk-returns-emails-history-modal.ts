...
@Component({
  selector: 'app-bulk-returns-emails-history-modal',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [TranslatePipe, Modal, BulkHistoryTab, BatchRequestsModal, RequestErrorModal],
  templateUrl: './bulk-returns-emails-history-modal.html',
})
export class BulkReturnsEmailsHistoryModal {
  ...
  private loadBatchesByPage(page: number): void {
    ...
    this.batchService.getCustomerReturnsEmailsBatches(this.historyPageSize(), safePage)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('CUSTOMERS_PAGE.BULK_HISTORY_LOAD_ERROR'));
        }
      });
  }
    ...
    this.batchService.getBatchDetail(batchId, this.detailErrorsPerPage)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('CUSTOMERS_PAGE.BULK_HISTORY_LOAD_DETAIL_ERROR'));
        }
      });
  }
    ...
    this.batchService.getBatchRequests(batchId, this.batchRequestsPageSize(), safePage, this.batchRequestsStatusFilter())
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('CUSTOMERS_PAGE.BULK_HISTORY_LOAD_REQUESTS_ERROR'));
        }
      });
  }
