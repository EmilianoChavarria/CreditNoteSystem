...
import { WorkflowDetail } from '../workflow-history-drawer/workflow-history-drawer';

@Component({
  selector: 'app-workflow-history-modal',
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './workflow-history-modal.html',
  imports: [LucideAngularModule, TranslateModule, TranslatePipe, Modal],
})
export class WorkflowHistoryModal {
  ...
  getStepMarkerClasses(statusKey: string): string {
    switch (statusKey) {
      ...
      case 'rejected':
      case 'cancelled': return 'border-red-400 bg-red-50 text-red-600';
