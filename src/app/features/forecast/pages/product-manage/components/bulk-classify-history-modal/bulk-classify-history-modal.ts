...
import { BulkHistoryTab } from '../../../../../../features/requests/components/batchs/bulk-history-tab/bulk-history-tab';
import { BatchRequestsModal } from '../../../../../../features/requests/components/batchs/batch-requests-modal/batch-requests-modal';
import { RequestErrorModal } from '../../../../../../features/requests/components/batchs/request-error-modal/request-error-modal';
...
@Component({
  selector: 'app-bulk-classify-history-modal',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [TranslatePipe, Modal, BulkHistoryTab, BatchRequestsModal, RequestErrorModal],
  templateUrl: './bulk-classify-history-modal.html',
})
export class BulkClassifyHistoryModal {
  ...
  private loadBatchesByPage(page: number): void {
    ...
    this.batchService.getProductClassificationBatches(this.historyPageSize(), safePage)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.BULK_CLASSIFY.LOAD_HISTORY_ERROR'));
        }
      });
  }
    ...
    this.batchService.getBatchDetail(batchId, this.detailErrorsPerPage)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.BULK_CLASSIFY.LOAD_DETAIL_ERROR'));
        }
      });
  }
    ...
    this.batchService.getBatchRequests(batchId, this.batchRequestsPageSize(), safePage)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.BULK_CLASSIFY.LOAD_REQUESTS_ERROR'));
        }
      });
  }
  ...
  private mapRequestItemToHistoryRow(item: BatchRequestItem): RequestHistoryRow {
    ...
    const idProducto = rawData['idProducto'] ?? rawData['idproducto'];
    const requestNumber = String(request['requestNumber'] ?? idProducto ?? item.requestId ?? item.id ?? '-');
