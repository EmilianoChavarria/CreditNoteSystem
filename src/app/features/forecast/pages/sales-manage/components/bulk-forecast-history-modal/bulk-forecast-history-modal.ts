...
@Component({
  selector: 'app-bulk-forecast-history-modal',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [TranslatePipe, Modal, BulkHistoryTab, BatchRequestsModal, RequestErrorModal],
  templateUrl: './bulk-forecast-history-modal.html',
})
export class BulkForecastHistoryModal {
  ...
  private loadBatchesByPage(page: number): void {
    ...
    this.batchService.getForecastBatches(this.historyPageSize(), safePage)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.SALES_MANAGE.BULK_HISTORY_LOAD_ERROR'));
        }
      });
  }
    ...
    this.batchService.getBatchDetail(batchId, this.detailErrorsPerPage)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.SALES_MANAGE.BULK_HISTORY_LOAD_DETAIL_ERROR'));
        }
      });
  }
    ...
    this.batchService.getBatchRequests(batchId, this.batchRequestsPageSize(), safePage, this.batchRequestsStatusFilter())
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.SALES_MANAGE.BULK_HISTORY_LOAD_REQUESTS_ERROR'));
        }
      });
  }
  ...
  private mapRequestItemToHistoryRow(item: BatchRequestItem): RequestHistoryRow {
    ...
    const clientNumber = rawData['idCliente'] ?? rawData['customerNumber'] ?? rawData['clientNumber'] ?? rawData['client_number'];
    const requestNumber = String(request['requestNumber'] ?? clientNumber ?? item.requestId ?? item.id ?? '-');
    const displayName = (rawData['razonSocial'] || rawData['businessName'] || rawData['business_name'])
      ? String(rawData['razonSocial'] ?? rawData['businessName'] ?? rawData['business_name'])
      : undefined;
