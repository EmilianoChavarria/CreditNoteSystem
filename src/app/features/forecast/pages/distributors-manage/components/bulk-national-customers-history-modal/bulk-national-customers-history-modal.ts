...
@Component({
  selector: 'app-bulk-national-customers-history-modal',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [TranslatePipe, Modal, BulkHistoryTab, BatchRequestsModal, RequestErrorModal],
  templateUrl: './bulk-national-customers-history-modal.html',
})
export class BulkNationalCustomersHistoryModal {
  ...
  private loadBatchesByPage(page: number): void {
    ...
    this.batchService.getNationalCustomersBatches(this.historyPageSize(), safePage)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.DISTRIBUTORS.NATIONAL_BULK_HISTORY_LOAD_ERROR'));
        }
      });
  }
    ...
    this.batchService.getBatchDetail(batchId, this.detailErrorsPerPage)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.DISTRIBUTORS.NATIONAL_BULK_HISTORY_LOAD_DETAIL_ERROR'));
        }
      });
  }
    ...
    this.batchService.getBatchRequests(batchId, this.batchRequestsPageSize(), safePage, this.batchRequestsStatusFilter())
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(this.translateService.instant('FORECAST.DISTRIBUTORS.NATIONAL_BULK_HISTORY_LOAD_REQUESTS_ERROR'));
        }
      });
  }
  ...
  private mapRequestItemToHistoryRow(item: BatchRequestItem): RequestHistoryRow {
    ...
    const clientNumber = rawData['customerNumber'] ?? rawData['clientNumber'] ?? rawData['client_number'];
    const requestNumber = String(request['requestNumber'] ?? clientNumber ?? item.requestId ?? item.id ?? '-');
    const displayName = (rawData['emails'])
      ? String(rawData['emails'])
      : undefined;
